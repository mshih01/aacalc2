export function preparse(isnaval, input, attdef, skipAA = false) {
...
export function count_units(input, tok) {
    let cnt = 0;
...
export function preparse_token(input) {
    const space = ' ';
    const comma = ',';
...
function preparse_artillery(input, attdef) {
    if (attdef != 0) {
        return input;
    }
    let out = input;
...
function preparse_skipaa(input) {
    let out = '';
...
export function preparse_battleship(input) {
    // remove "E"
    let removeE = '';
    for (const ch of input) {
        if (ch == 'E') {
