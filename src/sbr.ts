...
export function sbr(input: sbr_input): aacalc_output {
  let problem = new SbrProblem(
    input.numBombers,
    input.industrialComplexHitPoints,
    input.diceMode,
    input.verboseLevel,
    input.inProgress,
    input.pruneThreshold,
    input.reportPruneThreshold,
  );
  problem.solve();
  ...
  let output: aacalc_output = {
    attack: {
      ...
      ipcLoss: [att_ipc, 0, 0],
      incrementalLoss: [att_ipc, 0, 0],
    },
    defense: {
      survives: [prob_def_survives, 0, 0],
      ipcLoss: [def_ipc, 0, 0],
      incrementalLoss: [def_ipc, 0, 0],
    },
    casualtiesInfo: [],
    att_cas: att_cas,
    def_cas: def_cas,
