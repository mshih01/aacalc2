...
// retreat_expected_ipc_profit_threshold, ev_future_wave
type Setting2 = [
  number | undefined, // retreat threshold
  boolean,
];
let settings: Setting2[] = [
  [0, true],
  [undefined, false],
];
...
for (let ii = 0; ii < settings.length; ii++) {
  let [retreat_expected_ipc_profit_threshold, ev_future_wave] = settings[ii];
  for (let i = 0; i < inputSettings.length; i++) {
    let [setting, fileindex] = inputSettings[i];
    let [descriptionIn, retreat, round, strafe, is_deadzone, territory_value, do_roundless_eval] =
      setting;
...
*/
      ...
      wave_info: [
        {
          ...
          rounds: -1,
          retreat_threshold: 0,
          retreat_expected_ipc_profit_threshold: retreat_expected_ipc_profit_threshold,
          retreat_strafe_threshold: strafe,
    ...
    let description = descriptionIn + ii;
