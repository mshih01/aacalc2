import { multiwaveExternal, multiwaveComplexity, multiwaveComplexityFastV2, sbrExternal, armyRecommend, } from '../index.js';
let out = [];
...
for (let i = 0; i < inputSettings.length; i++) {
    ...
    let [description, retreat, pwin_retreat, pwin_mode, round, strafe, is_deadzone, territory_value, do_roundless_eval, report_complexity_only, retreat_lose_air_probability,] = setting;
    // 1 vs. 1
    const input = {
        wave_info: [
            {
...
let padding = 10;
let heading = 'profit'.padEnd(padding) +
    ' ' +
...
if (false) {
    ...
    let inputSettings = inputSettings2;
    /*
  ...
  */
    ...
    let precision = 3;
    for (let i = 0; i < inputSettings.length; i++) {
        let [description, retreat, round, strafe, is_deadzone, territory_value, roundless, retreat_air,] = inputSettings[i];
        const input = {
            wave_info: [
                {
                    attack: {
...
}
{
    ...
    for (let i = 0; i < out.length; i++) {
        let o = out[i];
        console.log(o[0], JSON.stringify(o[1]), 'cost', o[2], 'runtime', o[3].toFixed(1));
    }
}
//# sourceMappingURL=maketests.js.map