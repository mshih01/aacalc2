...
function compute_prob_table(N, p, prune_threshold) {
    let P = [];
    let maxsum = N * 6;
    let minsum = N * 1;
    P[0] = [];
...
class SbrProblem {
    ...
    um;
    prune_threshold;
    report_prune_threshold;
    P; // P[i][j] is the probability that i bombers gets j hits.
    phit; // P[i][j] is the probability that i bombers gets j hits.
    Patt; // probability distribution of attackers (result of aa hits)
    // Patt[i] is the probability that i bombers left
    Pdef; // probability distribution of defenders (result of sbr run)
    profitDistribution = {};
    // Pdef[i] is the probability that i defending hit points left.
    constructor(numBombers, numIPCHitPoints, diceMode, verbose_level, in_progress, prune_threshold, report_prune_threshold) {
...
export function sbr(input) {
    let problem = new SbrProblem(input.numBombers, input.industrialComplexHitPoints, input.diceMode, input.verboseLevel, input.inProgress, input.pruneThreshold, input.reportPruneThreshold);
    problem.solve();
    let prob_att_survives = 0.0;
    let att_ipc = 0.0;
    let stat = problem.um.get_stat('b');
    let att_cas = [];
    for (let i = 0; i < problem.Patt.length; i++) {
        // bombers remain
    ...
    let def_ipc = 0.0;
    let def_cas = [];
    for (let i = 0; i < problem.Pdef.length; i++) {
        // ipc's remain
