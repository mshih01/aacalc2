...
import { remove_subhits2, remove_planehits2, remove_navalhits2, remove_dlast_subhits2, remove_dlast_planehits2, remove_dlast_navalhits2, hasDestroyer, } from './unitgroup.js';
// iterate all possible next states -- and update the probabilities
export function solve_one_general_state_copy1(problem, N, M, allow_same_state, numBombard, do_retreat_only, disable_retreat, onNextState = (problem, ii, prob, n, m, num_rounds) => {
    problem.P_1d[ii] += prob;
    // problem.setiP(ii, problem.getiP(ii) + prob);
}, onInitState = (problem, n, m) => problem.getP(n, m), onExitState = (problem, n, m) => { }) {
    ...
    // prob no hits.
    let P0 = att_sub.get_prob_table(N1, 0) *
    ...
    if (N1 > 0 && N2 == 0 && N3 == 0 && M2 > 0) {
        if (!def_destroyer) {
            ...
            const p = att_sub.get_prob_table(N1, 0) *
                def_sub.get_prob_table(M1, 0) *
                def_naval.get_prob_table(M3, 0) *
    ...
    if (M1 > 0 && M2 == 0 && M3 == 0 && N2 > 0) {
        if (!att_destroyer) {
            const p = def_sub.get_prob_table(M1, 0) *
    ...
    if (N1 == 0 && N2 > 0 && N3 == 0 && M1 > 0) {
        const p = att_air.get_prob_table(N2, 0) *
            def_naval.get_prob_table(M3, 0) *
            def_air.get_prob_table(M2, 0) *
    ...
    if (M1 == 0 && M2 > 0 && M3 == 0 && N1 > 0) {
        const p = def_air.get_prob_table(M2, 0) *
            att_naval.get_prob_table(N3, 0) *
            att_air.get_prob_table(N2, 0) *
    ...
    else if (enable_airsub_optimization &&
        problem.rounds < 0 &&
        !problem.is_retreat &&
        problem.is_naval &&
        N3 == 0 &&
...
//# sourceMappingURL=solveone1.js.map