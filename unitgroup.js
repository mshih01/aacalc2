...
export class unit_group_manager {
    unit_group_arr;
    mymap;
    get_or_create_unit_group(um, input, attdef, diceMode) {
        let ug;
...
export class unit_manager {
    ...
    verbose_level;
    skip_compute = false;
    constructor(verbose_level, skip_compute = false) {
    ...
    make_unit(fullname, ch, ch2, att, def, cost, hits, isLand, isSub, isDestroyer, isAir, isAA, isBombard, isAmphibious) {
...
export class unit_group {
    ...
    seti_prob_table(ii, val) {
        this.prob_table2[ii] = val;
    }
    set_prob_table(i, j, val) {
        const ii = this.getIndex(i, j);
        this.prob_table2[ii] = val;
    }
    constructor(manager, input_str, attdef, diceMode) {
        ...
        const biasedDiceProb = [0, 1, 3, 6, 8, 9, 10];
...
export class general_unit_graph_node {
    ...
    num_dest;
    num_aa;
    hasLand = false; // true if unit_str has land units.
    cost;
    deadzone_cost; // land units taking territories in a deadzone.
    firstAirCasualty; // index of the first air casualty, -1 if no air units.
    dlast = false;
    index = 0;
    next_aahit = undefined;
    next_subhit = undefined;
    next_airhit = undefined;
    next_navalhit;
    next_dlast_subhit = undefined;
    next_dlast_airhit = undefined;
    next_dlast_navalhit = undefined;
    next_submerge = undefined;
    next_retreat_amphibious = undefined;
    next_crash_fighters = undefined;
    next_remove_noncombat = undefined;
    naaArr = [];
    nsubArr = [];
    nairArr = [];
    nnavalArr = [];
    ...
    ndlastairArr = [];
    ndlastnavalArr = [];
    nosub_group = undefined;
    naval_group = undefined;
    numBB = 0;
    constructor(um, unit_str, retreat, is_nonaval) {
...
export class general_unit_group {
    ...
    dlast_group;
    nodeArr = [];
    constructor(um, input_str, attdef, dest_last, submerge, max_remove_hits, numAA, cas, is_nonaval, is_amphibious, crash_fighters, diceMode, is_naval) {
        this.um = um;
        this.unit_str = input_str;
        this.attdef = attdef;
...
export class unit_stat {
    ...
    isBombard;
    isAmphibious;
    constructor(fullname, ch, ch2, att, def, cost, hits, isLand, isSub, isDestroyer, isAir, isAA, isBombard, isAmphibious) {
...
export function hasDestroyer(group, node) {
...
export function remove_subhits2(node, hits) {
    const n = hits;
    return node.nsubArr[n];
}
export function remove_aahits(group, hits, index) {
    const node = group.nodeArr[index];
    const n = hits;
    return node.naaArr[n];
}
export function remove_dlast_subhits2(node, hits) {
    const n = hits;
    return node.ndlastsubArr[n];
}
export function remove_planehits2(node, hasDest, hits) {
    const n = hits;
    if (hasDest) {
...
export function remove_dlast_planehits2(node, hasDest, hits) {
    const n = hits;
    if (hasDest) {
...
export function remove_navalhits2(node, hits) {
    const n = hits;
    return node.nnavalArr[n];
}
export function remove_dlast_navalhits2(node, hits) {
    const n = hits;
    return node.ndlastnavalArr[n];
}
export function isAir(um, input) {
    const stat = um.get_stat(input);
    return stat.isAir;
}
export function isSub(um, input) {
    const stat = um.get_stat(input);
    return stat.isSub;
}
export function isDestroyer(um, input) {
    const stat = um.get_stat(input);
    return stat.isDestroyer;
}
function isTransport(um, input) {
    return input == 'T';
}
export function isLand(um, input) {
    const stat = um.get_stat(input);
    return stat.isLand;
}
export function hasLand(um, input) {
...
export function hasAmphibious(um, input) {
    for (let i = input.length - 1; i >= 0; i--) {
        const ch = input.charAt(i);
...
export function hasNonAAUnit(um, input) {
    for (let i = input.length - 1; i >= 0; i--) {
        const ch = input.charAt(i);
        const stat = um.get_stat(ch);
        if (!stat.isAA) {
...
export function remove_one_plane(um, input_str) {
...
export function remove_one_notdestroyer(um, input_str) {
    const N = input_str.length;
    for (let i = N - 1; i >= 0; i--) {
        const ch = input_str.charAt(i);
...
export function remove_one_notplane(um, input_str, skipd) {
    const N = input_str.length;
    for (let i = N - 1; i >= 0; i--) {
        const ch = input_str.charAt(i);
        if (!isAir(um, ch) &&
...
export function remove_one_notsub(um, input_str, skipd) {
    const N = input_str.length;
    for (let i = N - 1; i >= 0; i--) {
        const ch = input_str.charAt(i);
        if (!isSub(um, ch) &&
...
export function retreat_subs(um, input_str) {
    const N = input_str.length;
    let out = '';
    let subs = '';
    let num_subs = 0;
...
export function retreat_non_amphibious(um, input_str) {
    const N = input_str.length;
    let out = '';
    let retreat = '';
    for (let i = 0; i < N; i++) {
...
export function get_deadzone_cost_from_str(um, s, retreat = '') {
    let cost = 0;
    let i;
...
export function getFirstAirCasualty(um, unit_str) {
...
function get_cost_remain(um, group, ii) {
    let cost = 0;
    let i;
    for (i = 0; i < ii; i++) {
...
export function get_general_cost_remain(um, group, ii) {
...
export function get_cost_from_str(um, s, retreat = '') {
    let cost = 0;
    let i;
    for (i = 0; i < s.length; i++) {
        const ch = s.charAt(i);
...
function make_node_key(s, retreat) {
    return s + ';' + retreat;
}
// compute all possible sub-states (and all possible casualties from every state).
export function compute_remove_hits(naval_group, max_remove_hits, numAA, cas, is_amphibious) {
    ...
    const mycompare = (a, b) => b.cost - a.cost;
    ...
    if (numAA > 0 && naval_group.attdef == 0) {
        ...
        if (num_planes < num_shots)
            num_shots = num_planes;
    ...
    if (cas != undefined) {
        for (let i = 0; i < cas.length; i++) {
            ...
            if (ii == undefined) {
                const newnode = new general_unit_graph_node(naval_group.um, s, '', naval_group.is_nonaval);
    ...
    while (myheap.length > 0) {
        ...
        const ii = mymap.get(key);
        if (ii == undefined) {
            newnode = new general_unit_graph_node(naval_group.um, s, node.retreat, naval_group.is_nonaval);
            if (newnode.num_naval > 0) {
                newnode.dlast = node.dlast;
            }
            else {
        ...
        if (!naval_group.is_naval || !isAir(naval_group.um, ch)) {
            ...
            const ii = mymap.get(key);
            if (ii == undefined) {
                node2 = new general_unit_graph_node(naval_group.um, s2, node.retreat, naval_group.is_nonaval);
    ...
    */
        ...
        if (naval_group.submerge_sub &&
            ...
            const ii = mymap.get(key);
            if (ii == undefined) {
                node2 = new general_unit_graph_node(naval_group.um, s2, subs, naval_group.is_nonaval);
                if (node2.num_naval > 0) {
                    node2.dlast = node.dlast;
                }
                else {
        ...
        if (is_amphibious && node.unit_str.length > 0 && node.retreat.length == 0) {
            ...
            //if (true || hasAmphibious (naval_group.um, s2))
            {
                ...
                const ii = mymap.get(key);
                if (ii == undefined) {
                    node2 = new general_unit_graph_node(naval_group.um, s2, amphibious, naval_group.is_nonaval);
                    if (node2.num_naval > 0) {
                        node2.dlast = node.dlast;
                    }
                    else {
    ...
    for (i = 0; i < naval_group.nodeArr.length; i++) {
        ...
        let prev = undefined;
        let node2;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_subhit) {
            prev = node2;
        ...
        node.nairArr = [];
        prev = undefined;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_airhit) {
            prev = node2;
        ...
        prev = undefined;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_navalhit) {
            prev = node2;
        ...
        node.ndlastsubArr = [];
        prev = undefined;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_dlast_subhit) {
            prev = node2;
        ...
        node.ndlastairArr = [];
        prev = undefined;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_dlast_airhit) {
            prev = node2;
        ...
        // compute nextnaval array
        prev = undefined;
        for (node2 = node; node2 != undefined && node2 != prev; node2 = node2.next_dlast_navalhit) {
            prev = node2;
    ...
    for (let i = 0; i < naval_group.nodeArr.length; i++) {
        ...
        if (naval_group.um.verbose_level > 2) {
            ...
            if (naval_group.um.verbose_level > 4) {
                console.log(node.unit_str, node.num_subs, node.num_air, node.num_naval, naval_group.sub_group.unit_str.substr(0, node.num_subs), naval_group.naval_group.unit_str.substr(0, node.num_naval), node.naval_group != undefined
...
export function make_unit_group(um, input_str, attdef, diceMode) {
    if (um.skip_compute) {
