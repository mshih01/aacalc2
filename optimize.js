...
import { getSubArmies, multiEvalExternal, multiwaveExternal, getArmyCost, getIntegersInRange, getCombinations, } from './external.js';
import {} from './solve.js';
function initArmy(army, value) {
    let result = { ...army };
    ...
    switch (solveType) {
        case 'multiEval': {
            ...
            multiEvalResult.sort((a, b) => {
                const av = Number(a[2] < surviveThreshold) ? 1.0 : 0.0;
                const bv = Number(b[2] < surviveThreshold) ? 1.0 : 0.0;
                if (av != bv) {
                    return bv - av;
                }
                else {
        ...
        case 'exhaust': {
            ...
            for (let i = 0; i < armies.length; i++) {
                ...
                const survive = attDefType == 'defender'
            ...
            out.sort((a, b) => {
                const av = Number(a[1] < surviveThreshold) ? 1.0 : 0.0;
                const bv = Number(b[1] < surviveThreshold) ? 1.0 : 0.0;
                if (av != bv) {
                    return bv - av;
                }
                else {
        ...
        case 'fuzzyBinarySearch':
            armies.sort((a, b) => {
                const av = attDefType == 'attacker' ? Number(a[2]) : Number(a[3]);
                const bv = attDefType == 'attacker' ? Number(b[2]) : Number(b[3]);
                if (av != bv) {
                    return av - bv;
                }
                else {
            ...
            armies.sort((a, b) => {
                const av = attDefType == 'attacker' ? Number(a[2]) : Number(a[3]);
                const bv = attDefType == 'attacker' ? Number(b[2]) : Number(b[3]);
                const acost = av >= bestPower ? 0 : 1;
                const bcost = bv >= bestPower ? 0 : 1;
                if (acost != bcost) {
                    return acost - bcost;
                }
                else {
        ...
        case 'gridSearch': {
            ...
            const mymap = new Map();
            let callCount = 0;
            function getArmy(vars) {
                // vars: [inf, art, arm, fig, bom, aa]
                const army = attDefType == 'defender'
                    ? {
            ...
            function armyCostObjective(vars) {
            ...
            function armyCostObjectiveHelper(vars) {
                // vars: [inf, art, arm, fig, bom, aa]
            ...
            const numInf = maxArmy['inf'] ?? 0;
            const numArt = maxArmy['art'] ?? 0;
            const numArm = maxArmy['arm'] ?? 0;
            const numFig = maxArmy['fig'] ?? 0;
            const numBom = maxArmy['bom'] ?? 0;
            const numCru = maxArmy['cru'] ?? 0;
            const numBat = maxArmy['bat'] ?? 0;
            const numAA = maxArmy['aa'] ?? 0;
            const initial = attDefType == 'defender'
                ? [numInf, numArt, numArm, numFig, numBom, numAA]
                : [numInf, numArt, numArm, numFig, numBom, numBat, numCru];
            const bounds = attDefType == 'defender'
                ? [
                    [0, numInf],
                    [0, numArt],
                    [0, numArm],
            ...
            const finalParams = solveType == 'gridSearch'
...
function approximateGradient(initialParams, bounds, delta, // Small change for approximating gradient
objectiveFunction) {
...
function doNeighborSearch(objectiveFn, x, bound, delta) {
    let range = [];
...
function doGridSearch(objectiveFn, bound) {
    let range = [];
...
function gridSearch(objectiveFn, bound) {
    let currBound = [];
    for (let i = 0; i < bound.length; i++) {
        let low = bound[i][0];
        let high = bound[i][1];
...
function lineSearch(objectiveFn, x, bound, numIterations) {
    let x0 = x.slice();
    for (let i = 0; i < numIterations; i++) {
        let gradientAtX = approximateGradient(x0, bound, 1.0, objectiveFn);
        // negative gradient
        let direction = gradientAtX.map((val, i) => val > 0 ? -val : -val);
        ...
        let stepSize = backtrackingLineSearch(objectiveFn, x0, bound, direction, 0.5, 0.5, 2.0);
        const newX = x0.map((val, i) => {
...
function backtrackingLineSearch(objectiveFn, x, // Current point
bound, // Current point
direction, // Search direction (e.g., negative gradient)
alpha = 0.5, // Armijo condition parameter (0 < alpha < 1)
beta = 0.8, // Backtracking reduction factor (0 < beta < 1)
initialStepSize = 1.0) {
    ...
    while (true) {
        ...
        let xNew = x.map((val, i) => Math.round(val + stepSize * direction[i]));
        for (let i = 0; i < xNew.length; i++) {
            let max = bound[i][1];
            let min = bound[i][0];
            if (xNew[i] > max) {
