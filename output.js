...
import { get_general_cost_remain, unit_manager, unit_group, general_unit_group, hasLand, } from './unitgroup.js';
export function report_filter(threshold, p) {
    if (p < threshold) {
        return 0;
    }
    return p;
}
export function collect_results(parent_prob) {
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < M; j++) {
            ...
            if (p > 0) {
                ...
                const dzcost = problem.is_deadzone && problem.def_data.nodeArr[j].N == 0
                    ? problem.att_data.nodeArr[i].deadzone_cost
                    : 0;
                const takes = problem.def_data.nodeArr[j].N == 0 &&
                    problem.att_data.nodeArr[i].hasLand;
                const territoryValue = takes ? problem.territory_value : 0;
                const cost3 = parent_prob.sortMode == 'unit_count'
                    ? cost
...
function get_general_group_string(um, group, sz) {
    let out = '';
...
export function get_reduced_group_string(input) {
    const map = new Map();
...
*/
function get_cost(um, group, ii, cas = '', skipBombard = false) {
...
export function get_general_cost(problem, group, ii) {
...
export function print_general_results(baseproblem, resultArr) {
    ...
    const att_map = new Map();
    const att_retreat_map = new Map(); // for retreating units
...
export function collect_and_print_results(problem) {
